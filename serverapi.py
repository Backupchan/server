...
class ServerAPI:
    ...
    def edit_target(self, target_id: str, new_name: str, new_recycle_criteria: str, new_recycle_value: int, new_recycle_action: str, new_type: str, new_location: str, new_name_template: str, deduplicate: bool, alias: str | None, min_backups: int | None, tags: list[str] | None):
        with self.lock:
            target = self.db.get_target(target_id)
            old_location = target.location
            old_name_template = target.name_template
            self.db.edit_target(target_id, new_name, new_recycle_criteria, new_recycle_value, new_recycle_action, new_type, new_location, new_name_template, deduplicate, alias, min_backups, tags)
    ...
    def delete_target_recycled_backups(self, target_id: str, delete_files: bool):
        with self.lock:
